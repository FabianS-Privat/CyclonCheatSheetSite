---
sidebar_position: 7
---

# Grim Tangle

## Zone Read

:::tip Simple Read
Follow the Edge behind the Checkpoint in the relative Direction of the Cemetery of Grim Tangle
:::

There are 4 Layout Categories, based on [Angormus](https://www.youtube.com/@Angormus) definition of the Zone. Regardless if Zone goes from Left to Right, Right to Left, Top to Bottom or Bottom to Top, All Layouts fit into on of the Categories.

- Spiral
- - The Checkpoint Edge leads to Boss Arena behind the Entrance and the Exit is Diagonal in the Direction of the Cemetery
- Hook
- - Zone starts similar to Spiral, but doesnt loop behind Entrance, continues in the Direction of the Cemetery
- V-Shaped?
- - Takes a V ur U Curve depending on Perspective and the Druid can be found in a Split Path near the Exit
- Straight
- - Long straight Wall at the Beginning of the Zone before first Branching of the Path. Stick to Waypoint Edge.

```mermaid
graph TD;
    A[Is Straight Wall at Start?]-->|yes|B[Straigth Variant, follow the WP Edge]
    A-->|no|C[Follow the Checkpoint Edge]
    B-->D[Move into the Direction of Cemetery to Grim Tangle, if Path Splits stick to the Edge]
    C-->D
    D-->|Found Druid first|E[Continue follow the Edge to the Exit]
    D-->|Found Exit first|F[Head back to last Split Path and take the other Path to find Druid ]
```

## Layouts

### Overworld Examples

| Cemetery Left of Tangle                                                               | Cemetery Right of Tangle                                                              |
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| ![Grim Tangle Overworld 1](<./img/Grim%20Tangle/Grim%20Tangle%20Overworld%20(2).png>) | ![Grim Tangle Overworld 2](<./img/Grim%20Tangle/Grim%20Tangle%20Overworld%20(1).png>) |

### Variant Table

| Spiral                                                          | Hook                                                            | V-Shaped?                                                       | Straight                                                        |
| --------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------- |
| ![Grim Tangle 1](<./img/Grim%20Tangle/Grim%20Tangle%20(2).png>) | ![Grim Tangle 2](<./img/Grim%20Tangle/Grim%20Tangle%20(3).png>) | ![Grim Tangle 3](<./img/Grim%20Tangle/Grim%20Tangle%20(1).png>) | ![Grim Tangle 4](<./img/Grim%20Tangle/Grim%20Tangle%20(6).png>) |
| ![Grim Tangle 5](<./img/Grim%20Tangle/Grim%20Tangle%20(5).png>) |                                                                 | ![Grim Tangle 7](<./img/Grim%20Tangle/Grim%20Tangle%20(4).png>) |                                                                 |
|                                                                 |                                                                 | ![Grim Tangle 6](<./img/Grim%20Tangle/Grim%20Tangle%20(7).png>) |

### Points of Interest

Waypoint at the Entrance from Grelwood
Ervig, the Rotten Druid - LvL 1 Support Gem
